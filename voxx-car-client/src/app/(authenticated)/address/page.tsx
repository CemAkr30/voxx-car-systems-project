...
export default function AdresPage() {
  ...
  useEffect(() => {
    ...
    const mockFirmalar: Firma[] = Array.from({ length: 50 }, (_, index) => {
      return {
        ...
        vergiDairesi: `${
          ["İstanbul", "Ankara", "İzmir", "Bursa", "Antalya"][index % 5]
        } Vergi Dairesi`,
    ...
    mockFirmalar.forEach((firma) => {
      ...
      for (let i = 0; i < adresSayisi; i++) {
        ...
        mockAdresler.push({
          ...
          adres: `${firma.adi} ${
            adresTipiLabels[adresTipleri[i % adresTipleri.length] as AdresTipi]
          } Adresi, No: ${Math.floor(Math.random() * 100) + 1}, Kat: ${
            Math.floor(Math.random() * 10) + 1
          }`,
          il: ["İstanbul", "Ankara", "İzmir", "Bursa", "Antalya"][
            (firma.id.length + i) % 5
          ],
          ilce: ["Kadıköy", "Çankaya", "Konak", "Nilüfer", "Muratpaşa"][
            (firma.id.length + i) % 5
          ],
      ...
      for (let i = 0; i < iletisimSayisi; i++) {
        ...
        switch (tip) {
          ...
          case IletisimTipi.FAKS:
            numara = `+90 ${Math.floor(500 + Math.random() * 500)} ${Math.floor(
              100 + Math.random() * 900
  ...
  const handleDeleteFirma = async () => {
    ...
    try {
      ...
      setAdresler((prev) =>
        prev.filter((item) => item.firmaId !== deletingFirma.id)
      );
      setIletisimler((prev) =>
        prev.filter((item) => item.firmaId !== deletingFirma.id)
      );
      ...
      // Firmayı sil
      setFirmalar((prev) =>
        prev.filter((item) => item.id !== deletingFirma.id)
      );
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      setAdresler((prev) =>
        prev.filter((item) => !selectedItems.includes(item.firmaId))
      );
      setIletisimler((prev) =>
        prev.filter((item) => !selectedItems.includes(item.firmaId))
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-2">
          Firma, adres ve iletişim bilgilerini yönetin
        </p>
      </div>
              ...
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox
                      checked={
                        selectedItems.length === paginatedFirmalar.length &&
                    ...
                    <TableRow key={firma.id}>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuItem
                              onClick={() => openDetailDialog(firma)}
                            >
                              <Eye className="h-4 w-4 mr-2" />
                              Detay
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() => openFirmaDialog(firma)}
                            >
                              <Edit className="h-4 w-4 mr-2" />
                              Düzenle
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() => openDeleteDialog(firma)}
      ...
      <Dialog open={isFirmaDialogOpen} onOpenChange={setIsFirmaDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingFirma ? "Firma Düzenle" : "Yeni Firma Ekle"}
            </DialogTitle>
            <DialogDescription>
              {editingFirma
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFirmaFormData({ ...firmaFormData, adi: e.target.value })
                }
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsFirmaDialogOpen(false)}
            >
              İptal
            </Button>
      ...
      <Dialog open={isAdresDialogOpen} onOpenChange={setIsAdresDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingAdres ? "Adres Düzenle" : "Yeni Adres Ekle"}
            </DialogTitle>
            <DialogDescription>
              {editingAdres
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setAdresFormData({ ...adresFormData, adres: e.target.value })
                }
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setAdresFormData({ ...adresFormData, il: e.target.value })
                  }
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setAdresFormData({ ...adresFormData, ilce: e.target.value })
                  }
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsAdresDialogOpen(false)}
            >
              İptal
            </Button>
      ...
      {/* İletişim Dialog */}
      <Dialog
        open={isIletisimDialogOpen}
        onOpenChange={setIsIletisimDialogOpen}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingIletisim ? "İletişim Düzenle" : "Yeni İletişim Ekle"}
            </DialogTitle>
            <DialogDescription>
              {editingIletisim
                ? "İletişim bilgilerini güncelleyin"
                : "Yeni bir iletişim bilgisi ekleyin"}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setIsIletisimDialogOpen(false)}
            >
              İptal
            </Button>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              "{deletingFirma?.adi}" firmasını silmek istediğinizden emin
              misiniz? Bu işlem geri alınamaz ve firmaya ait tüm adres ve
              iletişim bilgileri de silinecektir.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedItems.length} firmayı silmek istediğinizden emin misiniz?
              Bu işlem geri alınamaz ve firmalara ait tüm adres ve iletişim
              bilgileri de silinecektir.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Vergi No: {activeFirma?.vergiNo} | Vergi Dairesi:{" "}
              {activeFirma?.vergiDairesi}
            </DialogDescription>
          </DialogHeader>
            ...
            >
              ...
              <TabsContent value="adresler" className="space-y-4">
                ...
                {getFirmaAdresler(activeFirma.id).length > 0 ? (
                  <div className="space-y-4">
                    {getFirmaAdresler(activeFirma.id).map((adres) => (
                      <Card key={adres.id}>
                        <CardContent className="p-4">
                        ...
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-2">
                                <Badge
                                  className={iletisimTipiColors[iletisim.tip]}
                                >
                                  {iletisimTipiIcons[iletisim.tip]}
                                  <span className="ml-1">
                                    {iletisimTipiLabels[iletisim.tip]}
                                  </span>
                                </Badge>
                              </div>
