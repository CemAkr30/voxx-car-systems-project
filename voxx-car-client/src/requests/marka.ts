...
import type {CreateFirmaRequest, Firma} from "@/schemas/marka";
import {isAxiosError} from "axios";
import {toast} from "sonner";

export const getAllMarka = async (): Promise<Firma[]> => {
    const {data} = await axiosClient.get(`${urls.marka}`);
    return data;
};

export const createMarka = async (marka: CreateFirmaRequest): Promise<void> => {
    try {
        await axiosClient.post<Firma>(`${urls.marka}`, marka);
...
export const updateMarka = async (marka: Firma): Promise<void> => {
    try {
        await axiosClient.put<Firma>(`${urls.marka}/${marka.id}`, marka);
