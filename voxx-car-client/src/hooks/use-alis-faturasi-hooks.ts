...
import type {AlisFaturasi, CreateAlisFaturasiRequest} from "@/schemas/alis-faturasi";
...
export const useCreateAlisFaturasiMutation = (onSuccess?: () => void) => {
    return useMutation({
        mutationFn: async (alisFaturasi: CreateAlisFaturasiRequest): Promise<void> =>
            await createAlisFaturasi(alisFaturasi),
        onSuccess() {
            onSuccess?.();
        },
    });
...
export const useUpdateAlisFaturasiMutation = (onSuccess?: () => void) => {
    return useMutation({
        mutationFn: async (alisFaturasi: AlisFaturasi) => await updateAlisFaturasi(alisFaturasi),
        onSuccess() {
            onSuccess?.();
        },
    });
