...
const aracTipleri = ["Binek", "Ticari", "Kamyon", "Otobüs", "Motosiklet", "Traktör"]
const segmentler = ["A", "B", "C", "D", "E", "F", "SUV", "MPV", "Pickup"]
const kasaTipleri = ["Sedan", "Hatchback", "Station Wagon", "Coupe", "Convertible", "SUV", "Pickup", "Van"]
const lastikTipleri = ["Yaz", "Kış", "4 Mevsim", "Run Flat"]
const renkler = ["Beyaz", "Siyah", "Gri", "Mavi", "Kırmızı", "Yeşil", "Sarı", "Kahverengi", "Mor", "Turuncu"]
...
export function AracManagement() {
    ...
    useEffect(() => {
        ...
        const mockAraclar: Arac[] = Array.from({length: 127}, (_, index) => {
            ...
            const model = mockModeller.filter((m) => m.markaId === marka.id)[0] || mockModeller[0]
            ...
            return {
                ...
                filoyaGirisTarihi: new Date(2020 + (index % 4), index % 12, (index % 28) + 1).toISOString().split("T")[0],
                filoyaGirisKm: String(5000 + index * 1000),
                tescilTarihi: new Date(2015 + (index % 9), index % 12, (index % 28) + 1).toISOString().split("T")[0],
                trafigeCikisTarihi: new Date(2015 + (index % 9), (index + 1) % 12, (index % 28) + 1)
                    .toISOString()
                    .split("T")[0],
                garantisiVarMi: index % 3 === 0,
                garantiBitisTarihi: new Date(2025 + (index % 3), index % 12, (index % 28) + 1).toISOString().split("T")[0],
                garantiSuresiYil: (2 + (index % 4)).toString(),
                garantiKm: String(100000 + index * 10000),
                tramer: index % 4 === 0,
                tramerTutari: index % 4 === 0 ? 5000 + index * 100 : 0,
                sonKmTarihi: new Date(2024, 0, (index % 30) + 1).toISOString().split("T")[0],
                sonKm: String(50000 + index * 2000),
                sonYakitMiktari: String(40 + (index % 20)),
                kiralandiMi: index % 5 === 0,
                kiralandigiTarih:
                    index % 5 === 0 ? new Date(2023, index % 12, (index % 28) + 1).toISOString().split("T")[0] : "",
                kontratSuresi: index % 5 === 0 ? (12 + (index % 24)).toString() : "",
                kiralikBitisTarihi:
                    index % 5 === 0 ? new Date(2024 + (index % 2), index % 12, (index % 28) + 1).toISOString().split("T")[0] : "",
                kiralayanFirmaId: index % 5 === 0 ? firma.id : "",
                kiralayanFirmaAdi: index % 5 === 0 ? firma.adi : "",
                filoDurum: (index % 6) + 1,
                createdAt: new Date(2024, 0, (index % 30) + 1).toISOString().split("T")[0],
    ...
    useEffect(() => {
        const filtered = araclar.filter((arac) => {
            ...
            const markaMatch = selectedMarkaFilter === "all" || arac.markaId === selectedMarkaFilter
            const durumMatch = selectedDurumFilter === "all" || arac.filoDurum.toString() === selectedDurumFilter
            const tipMatch = selectedTipFilter === "all" || arac.aracTipi === selectedTipFilter
            const segmentMatch = selectedSegmentFilter === "all" || arac.segment === selectedSegmentFilter
            ...
            return searchMatch && markaMatch && durumMatch && tipMatch && segmentMatch && garantiMatch && kiralikMatch
        })
    ...
    const paginationInfo: PaginationInfo = {
        ...
        itemsPerPage,
        startItem: Math.min((currentPage - 1) * itemsPerPage + 1, filteredAraclar.length),
    ...
    const exportToExcel = () => {
        const csvContent =
            ...
            filteredAraclar
                .map(
                    (item) =>
                        `${item.plaka},${item.markaAdi},${item.modelAdi},${item.modelYili},${item.aracTipi},${item.segment},${item.motorNo},${item.sasiNo},${item.renk},${filoDurumLabels[item.filoDurum as keyof typeof filoDurumLabels]},${item.garantisiVarMi ? "Var" : "Yok"},${item.kiralandiMi ? "Var" : "Yok"},${item.sonKm}`,
                )
                .join("\n")
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                ...
                <Card>
                    <CardContent className="p-4">
                        <div className="text-2xl font-bold text-yellow-600">{stats.bakim}</div>
                        <p className="text-sm text-gray-600">Bakımda</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="p-4">
                        <div className="text-2xl font-bold text-red-600">{stats.arizali}</div>
                        <p className="text-sm text-gray-600">Arızalı</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="p-4">
                        <div className="text-2xl font-bold text-purple-600">{stats.kiralik}</div>
                        <p className="text-sm text-gray-600">Kiralık</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="p-4">
                        <div className="text-2xl font-bold text-indigo-600">{stats.garantili}</div>
                ...
                <CardContent>
                    ...
                    <div className="space-y-4 mb-6">
                        ...
                        <div className="flex items-center space-x-2 flex-wrap gap-2">
                            ...
                            <Select value={selectedDurumFilter} onValueChange={setSelectedDurumFilter}>
                            ...
                            <Select value={selectedTipFilter} onValueChange={setSelectedTipFilter}>
                                <SelectTrigger className="w-32">
                            ...
                            <TableHeader>
                                <TableRow>
                                    <TableHead className="w-12">
                                        <Checkbox
                                            checked={selectedItems.length === paginatedAraclar.length && paginatedAraclar.length > 0}
                                    ...
                                    <TableRow key={arac.id}>
                                        <TableCell>
                                            <Checkbox
                                                ...
                                                onCheckedChange={(checked) => handleSelectItem(arac.id, checked as boolean)}
                                            />
                                        </TableCell>
                                        <TableCell className="font-mono font-medium">{arac.plaka}</TableCell>
                                        <TableCell>
                                            <div>
                                        ...
                                        <TableCell>
                                            <div>
                                                ...
                                                <div className="text-sm text-gray-500">{arac.segment} Segment</div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <Badge
                                                className={filoDurumColors[arac.filoDurum as keyof typeof filoDurumColors]}>
                                                {filoDurumLabels[arac.filoDurum as keyof typeof filoDurumLabels]}
                                            </Badge>
                                        </TableCell>
                                        <TableCell>
                                            {arac.garantisiVarMi ? (
                                        ...
                                        <TableCell>
                                            <div className="flex items-center">
                                                ...
                                                <span
                                                    className="text-sm">{Number.parseInt(arac.sonKm).toLocaleString()} km</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent>
                                                    <DropdownMenuItem onClick={() => openDetailDialog(arac)}>
            ...
            <Dialog open={isFilterDialogOpen} onOpenChange={setIsFilterDialogOpen}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Araç listesini filtrelemek için kriterleri seçin</DialogDescription>
                    </DialogHeader>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <Label>Segment</Label>
                            <Select value={selectedSegmentFilter} onValueChange={setSelectedSegmentFilter}>
                                <SelectTrigger>
            ...
            <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {viewingArac?.plaka} - {viewingArac?.markaAdi} {viewingArac?.modelAdi}
                        </DialogDescription>
                    </DialogHeader>
                    {viewingArac && (
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <Card>
                                    <CardContent className="p-4">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            <div>
                                                <span className="text-gray-600">Plaka:</span> {viewingArac.plaka}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Marka/Model:</span> {viewingArac.markaAdi}{" "}
                                                {viewingArac.modelAdi}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Model Yılı:</span> {viewingArac.modelYili}
                                            </div>
                                            <div>
                                                <span className="text-gray-600">Araç Tipi:</span> {viewingArac.aracTipi}
                                            </div>
                                            <div>
                                                <span className="text-gray-600">Segment:</span> {viewingArac.segment}
                                            </div>
                                            <div>
                                                <span className="text-gray-600">Renk:</span> {viewingArac.renk}
                                            </div>
                                            <div>
                                                <span className="text-gray-600">Kasa Tipi:</span> {viewingArac.kasaTipi}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Lastik Tipi:</span> {viewingArac.lastikTipi}
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
                                ...
                                <Card>
                                    <CardContent className="p-4">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            <div>
                                                <span className="text-gray-600">Motor No:</span> {viewingArac.motorNo}
                                            </div>
                                            <div>
                                                <span className="text-gray-600">Şasi No:</span> {viewingArac.sasiNo}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Tescil Tarihi:</span> {viewingArac.tescilTarihi}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Trafiğe Çıkış:</span> {viewingArac.trafigeCikisTarihi}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Filoya Giriş:</span> {viewingArac.filoyaGirisTarihi}
                                            </div>
                                            <div>
                                                <span className="text-gray-600">Giriş KM:</span>{" "}
                                                {Number.parseInt(viewingArac.filoyaGirisKm).toLocaleString()}
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                <span
                                                    className="text-gray-600">Son Yakıt:</span> {viewingArac.sonYakitMiktari} L
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
                                    ...
                                    <CardContent className="p-4">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            <div>
                                                ...
                                                <Badge
                                                    className={`ml-2 ${viewingArac.garantisiVarMi ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                                                >
                                            ...
                                            {viewingArac.garantisiVarMi && (
                                                <>
                                                    <div>
                                                        <span
                                                            className="text-gray-600">Garanti Bitiş:</span> {viewingArac.garantiBitisTarihi}
                                                    </div>
                                                    <div>
                                                        <span
                                                            className="text-gray-600">Garanti Süresi:</span> {viewingArac.garantiSuresiYil} yıl
                                                    </div>
                                                    <div>
                                                        <span className="text-gray-600">Garanti KM:</span>{" "}
                                                        {Number.parseInt(viewingArac.garantiKm).toLocaleString()}
                                                    </div>
                                                </>
                                            )}
                                            <div>
                                                ...
                                                <Badge
                                                    className={`ml-2 ${viewingArac.tramer ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}`}
                                                >
                                ...
                                <Card>
                                    <CardContent className="p-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <span
                                                    className="text-gray-600">Kiralayan Firma:</span> {viewingArac.kiralayanFirmaAdi}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Kiralama Tarihi:</span> {viewingArac.kiralandigiTarih}
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Kontrat Süresi:</span> {viewingArac.kontratSuresi} ay
                                            </div>
                                            <div>
                                                <span
                                                    className="text-gray-600">Bitiş Tarihi:</span> {viewingArac.kiralikBitisTarihi}
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
